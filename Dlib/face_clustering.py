...
for root, dirs, files in os.walk(faces_folder_path):
    for dir in dirs:
        ...
        for f in glob.glob(os.path.join(root, dir, "*")):
            print("Processing file: {}".format(f))
            images_origin.append(cv2.imread(f))
        ...
        if not os.path.isdir(os.path.join(output_folder_path, dir)):
            os.makedirs(os.path.join(output_folder_path, dir))

        # Save the extracted faces
        print("Saving faces in largest cluster to output folder...")
        for i, index in enumerate(indices):
            # img, shape = images_origin[index]
            file_path = os.path.join(output_folder_path, dir, "face_" + str(i) + ".jpg")
            # print("Processing output_dir: {}".format(file_path))
            cv2.imwrite(file_path, images_origin[index])
